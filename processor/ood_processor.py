...
class LE_Processor(Processor):
    ...
    def load_model(self):
        ...
        # for name, param in self.model.online_network.named_parameters():
        #     # if name not in ['fc.weight', 'fc.bias']:
        #     param.requires_grad = False
        # # self.num_grad_layers = 2
        # if hasattr(self.model, 'online_network_motion'):
        #     for name, param in self.model.online_network_motion.named_parameters():
        #         # if name not in ['fc.weight', 'fc.bias']:
        #         param.requires_grad = False
        #     # self.num_grad_layers += 2
        # if hasattr(self.model, 'online_network_bone'):
        #     for name, param in self.model.online_network_bone.named_parameters():
        #         # if name not in ['fc.weight', 'fc.bias']:
        #         param.requires_grad = False
        #     # self.num_grad_layers += 2
        # print(self.model)
    ...
    def test(self, epoch):
        ...
        for data, label, _ in train_loader:
            ...
            train_label_frag.append(label.data.cpu().numpy())

        # randomly sample calibration set
        # import random
        # num_calibrate = int(len(train_result_frag)*0.1)
        ...
        for data, label, name in test_loader:
            ...
            data = data.float().to(self.dev, non_blocking=True)
            label = label.long().to(self.dev, non_blocking=True)
            # name = name.to(self.dev, non_blocking=True)

            # inference
            with torch.no_grad():
                output = self.model(data, view=self.arg.view)
            test_result_frag.append(output.data.cpu().numpy())
            test_name_frag.extend(name)
            test_label_frag.append(label.cpu().numpy())
        ...
        self.test_name = test_name_frag
        self.test_numeric_label = np.concatenate(test_label_frag)
        ...
        num_known_ood = int(len(ood_indices)*0.1)
        ood_indices = random.sample(list(ood_indices), num_known_ood)
        self.ood_known = self.test_result[ood_indices]

        test_indices = np.where(self.test_numeric_label == 0)[0]
        num_calibrate = int(len(test_indices)*0.1)
        calibrate_indices = random.sample(list(test_indices), num_calibrate)
        self.calibrate_result = self.test_result[calibrate_indices]
        ...
        # fpr95, auroc, aupr = get_skeleton_eval_results( ### get_skeleton_results for nturgbd
        ...
        fpr95, auroc, aupr = get_eval_results( ### get_skeleton_results for nturgbd
            np.copy(self.train_result),
            np.copy(self.calibrate_result),
            np.copy(self.ood_known),
            np.copy(self.test_result),
            np.copy(self.train_label),
            np.copy(self.test_name),
            np.copy(self.test_numeric_label)
        )
