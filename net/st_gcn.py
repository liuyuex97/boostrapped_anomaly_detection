...
from net.utils.tgcn import ConvTemporalGraphical, DGCNN
...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, hidden_dim, num_class, graph_args,
        ...
        A_global = torch.tensor(self.graph.A_global, dtype=torch.float32, requires_grad=False)
        ...
        # self.dgcnn_global = nn.ModuleList((
        #     DGCNN(in_channels*12, hidden_channels),
        #     DGCNN(hidden_channels, hidden_channels)
        # ))

        # self.fc = nn.Linear(hidden_dim+16, num_class)
    ...
    def forward(self, x):
        # data normalization
        N, C, T, V, M = x.size()
        # x_global = x.clone()[:, :, :, 10, :].view(N, -1, M)
        ...
        # for dgcnn in self.dgcnn_global:
        #     x_global = dgcnn(x_global)
        # x_global = F.adaptive_max_pool1d(x_global, 1).view(N, -1)
