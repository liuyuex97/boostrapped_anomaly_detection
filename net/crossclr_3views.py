...
class CrosSCLR(nn.Module):
    ...
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768,
        ...
        if not self.pretrain:
            ...
            self.encoder_q_motion.fc = Identity()
            self.encoder_q_bone.fc = Identity()

            ### finetuning for shanghai dataset ###
            ...
            self.m = momentum
            self.T = Temperature
            self.loss = SupConLoss(temperature=0.5) # param from SSD paper
    ...
    def update_ptr(self, batch_size):
        ...
        # if self.queue_ptr[0] == 1 or self.queue_ptr_motion[0] == 1 or self.queue_ptr_bone[0]:
        #     self.queue_ptr[0], self.queue_ptr_motion[0], self.queue_ptr_bone[0] = 0, 0, 0

    def forward(self, im_q, im_k=None, im_qm=None, im_qmf=None, labels=None, view='all', cross=False, topk=1, context=True):
        """
        Input:
        ...
        #### SupCon loss ####
        # logits = torch.cat([q.unsqueeze(1), k.unsqueeze(1)], dim=1)
        # logits_motion = torch.cat([q_motion.unsqueeze(1), k_motion.unsqueeze(1)], dim=1)
        # logits_bone = torch.cat([q_bone.unsqueeze(1), k_bone.unsqueeze(1)], dim=1)
        # # import pdb; pdb.set_trace() # check dimension of logits, should be Nx2xK
        # logits = self.loss(logits, labels)
        # logits_motion = self.loss(logits_motion, labels)
        # logits_bone = self.loss(logits_bone, labels)
        #####
        ...
        l_pos_mmotion = torch.einsum('nc,nc->n', [q_motion_mixed, k]).unsqueeze(-1)
        l_pos_mbone = torch.einsum('nc,nc->n', [q_bone_mixed, k]).unsqueeze(-1)
        l_neg_m = torch.einsum('nc, ck->nk', [q_mixed, self.queue.clone().detach()])
        l_neg_mmotion = torch.einsum('nc,ck->nk', [q_motion_mixed, self.queue_motion.clone().detach()])
        l_neg_mbone = torch.einsum('nc,ck->nk', [q_bone_mixed, self.queue_bone.clone().detach()])
        logits_m = torch.cat([l_pos_m, l_neg_m], dim=1)
        logits_mmotion = torch.cat([l_pos_mmotion, l_neg_mmotion], dim=1)
        logits_mbone = torch.cat([l_pos_mbone, l_neg_mbone], dim=1)
        logits_m /=self.T
        logits_mmotion /=self.T
        logits_mbone /=self.T

        l_pos_mf = torch.einsum('nc,nc->n', [q_mixedf, k]).unsqueeze(-1)
        l_pos_mmotionf = torch.einsum('nc,nc->n', [q_motion_mixedf, k]).unsqueeze(-1)
        l_pos_mbonef = torch.einsum('nc,nc->n', [q_bone_mixedf, k]).unsqueeze(-1)
        l_neg_mf = torch.einsum('nc, ck->nk', [q_mixedf, self.queue.clone().detach()])
        l_neg_mmotionf = torch.einsum('nc,ck->nk', [q_motion_mixedf, self.queue_motion.clone().detach()])
        l_neg_mbonef = torch.einsum('nc,ck->nk', [q_bone_mixedf, self.queue_bone.clone().detach()])
        logits_mf = torch.cat([l_pos_mf, l_neg_mf], dim=1)
        logits_mmotionf = torch.cat([l_pos_mmotionf, l_neg_mmotionf], dim=1)
        logits_mbonef = torch.cat([l_pos_mbonef, l_neg_mbonef], dim=1)
        logits_mf /=self.T
        logits_mmotionf /=self.T
        logits_mbonef /=self.T
        #####################
        ...
        return logits, logits_motion, logits_bone, logits_m, logits_mmotion, logits_mbone, logits_mf, logits_mmotionf, logits_mbonef, labels
